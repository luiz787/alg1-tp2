...
int main(int argc, char**argv) {
    ...
    auto greedyStart = std::chrono::high_resolution_clock::now(); // Inicia o relógio.
...
void solveWithGreedyAlgorithm(const uint32_t maximumCost, Island* islands, const uint32_t amountOfIslands) {
    ...
    for (uint32_t i = 0; i < amountOfIslands; ++i) {
        ...
        const auto possibleDaysInIsland = remainderMoney / island.getDailyCost();
...
void solveWithDynamicProgramming(const uint32_t maximumCost, Island* islands, const uint32_t amountOfIslands) {
    ...
    for (uint32_t i = 1; i <= amountOfIslands; i++) {
         ...
         */
        const auto currentIsland = islands[i - 1];
        for (uint32_t cost = 0; cost <= maximumCost; cost++) {
            if (currentIsland.getDailyCost() > cost) {
                lookupTable[i][cost] = lookupTable[i - 1][cost];
            } else {
                const auto costWithoutUsingCurrentIsland = lookupTable[i - 1][cost];
                const auto costUsingCurrentIsland = currentIsland.getDailyPunctuation() +
                        lookupTable[i - 1][cost - currentIsland.getDailyCost()];
    ...
    /*
     * "Backtracking" para determinar a quantidade de dias da viagem.
     */
